...
def load_dataset(TrainData_name,TestData_name,selectedTarget,Apply_Weight_loss = False):
    ...
    df['stance_no'] = df['stance'].apply(lambda x: mapping[x])
    test_df['stance_no'] = test_df['stance'].apply(lambda x: mapping[x])
    mapping = {'None': 'حياد', 'Favor': 'مع', 'Against':'ضد'}
...
class TweetEmotionDataset(Dataset):
  ...
  def __getitem__(self, index: int):
    ...
    encoding_stance = self.tokenizer.encode_plus(
      data_row.stance,
    ...
    return dict(
      ...
      labels = encoding_stance["input_ids"],
      labels_no = torch.tensor(data_row.stance_no, dtype=torch.long)
    )
